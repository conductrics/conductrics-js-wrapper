...
(function() {
  ...
  window.ConductricsJS = (function() {
    var debounce, qsformat, _batchSend;
    ...
    ConductricsJS.prototype.decision = function(agent, opts, cb) {
      ...
      return this.send(url, opts, null, true, function(res) {
        var selection, _ref1;
        if (cb == null) {
          return;
        }
    ...
    ConductricsJS.prototype.send = function(url, data, body, batchable, cb) {
      ...
      if (batchable && ((_ref = this.opts.batching) === 'auto' || _ref === 'manual')) {
        ...
        switch (batchItem.type) {
          case 'decisions':
            if (url.length > 2) {
              batchItem.choices = url.slice(2).join('/');
            }
            break;
          case 'goal':
            if (url.length > 2) {
              batchItem.goal = url[2];
            }
        }
    ...
    ConductricsJS.prototype.batchSend = function() {
      var batchData, url;
      url = ['-', 'batch'];
      batchData = this.batch.concat();
      if (!(batchData.length > 0)) {
        return;
      }
      this.batchStart();
      return this.send(url, {}, batchData, false, function(results) {
        var i, _results;
        ...
        _results = [];
        for (i in batchData) {
          if ((results[i] != null) && (batchData[i] != null)) {
            _results.push(batchData[i].cb(results[i].data));
          }
        }
        return _results;
      });
